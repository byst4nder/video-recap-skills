...
`--step script` 或续跑验证 `narration.json` 时生成的预检结果。它只检查写稿和时间安全，不评价成片质量。
...
常见 code：`invalid_time`、`empty_narration`、`time_overlap`、`outside_clip_plan`、`over_budget`、`incomplete_sentence`、`slot_too_short`。
